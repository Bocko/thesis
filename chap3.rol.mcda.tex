...
In the following, we will briefly describe each paradigm, showing the some of the main approaches alongside illustrative examples.
...
Other particular ILP problems which deals with variables that are restricted to be either O or 1 are called \textbf{0-1 linear programming}.

\begin{example}[Travelling salesman problem (TSP)]
This problem is probably the most known optimization problem. It can be formulated as follows: given $n$ cities and the distance between each pair of cities, we have to find the shortest tour that visits each city once and returns to the origin city. This problem can be formulated as an ILP problem.
\end{example}
...
\end{gathered}
\end{equation}
where ${f_1(x), f_2(x), \dots, f_m(x)}$ is a set of $m$ criteria that needs to be minimized.

\subsubsection{Ill-posed problem}

\subsubsection{Dominance principle}

\subsubsection{Pareto-dominance}


In a multi-criteria paradigm, the first step is to identify the set of alternatives, denoted $\mathcal{A}=\{a_1, a_2, \ldots, a_n\}$ and evaluation criteria, denoted $\mathcal{F}=\{f_1,f_2,\ldots, f_m\}$. From this set, the Pareto frontier, which is the set of non-dominated solutions, can be identified. This dominance principle is defined as follows:

\begin{definition}[Dominance \cite{BraMar2002}]
